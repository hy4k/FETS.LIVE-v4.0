...
const CandidateTracker = lazy(
  () => import("./components/CandidateTrackerPremium")
);
...
const ChecklistManagement = lazy(
  () => import("./components/checklist/ChecklistManager")
);
...
function AppContent() {
  ...
  const renderContent = () => {
    if (isMobile) {
      ...
      if (activeTab === "my-desk")
        return <MobileMyDesk setActiveTab={setActiveTab} />;
      ...
      if (activeTab === "checklist-management")
        return <ChecklistManagement currentUser={profile} />;
    ...
    const routeComponents: {
      ...
      "my-desk": {
        component: <MyDesk onNavigate={setActiveTab} />,
        name: "My Desk",
      },
      "staff-management": {
      ...
      "fets-intelligence": {
        component: <FetsIntelligence initialQuery={aiQuery} />,
    ...
    const currentRoute =
      routeComponents[activeTab] || routeComponents["command-center"];
    return (
      <LazyErrorBoundary
        key={activeTab}
      ...
      >
        <Suspense
          fallback={<PageLoadingFallback pageName={currentRoute.name} />}
        >
  ...
  const isFullscreenPage =
    activeTab === "my-desk" || activeTab === "mobile-ai-chat";

  return (
    <div
      className={`golden-theme min-h-screen h-screen flex flex-col overflow-hidden relative ${getBranchTheme(
        activeBranch
      )}`}
    >
      {isMobile && !isFullscreenPage && (
        <div className="h-safe-top bg-[#F6C845] w-full flex-none" />
      )}
      ...
      <div
        className={`flex-1 scroll-touch relative ${
          isFullscreenPage ? "" : isMobile ? "pt-0" : "pt-4 px-4 md:px-8 pb-8"
        }`}
      >
        {renderContent()}
      </div>
