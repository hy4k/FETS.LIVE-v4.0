...
export const CalendarAnalysis: React.FC<CalendarAnalysisProps> = ({
  onClose,
  activeBranch,
}) => {
  const { user } = useAuth();
  ...
  const [selectedView, setSelectedView] = useState<
    "overview" | "clients" | "overlaps"
  >("overview");

  // Elegant Neumorphic Styles with Rich Colors
  const neumorphicBase = "bg-gradient-to-br from-[#1a1f2e] to-[#0d1117]";
  const neumorphicCard =
    "bg-gradient-to-br from-[#1e2537] to-[#151b28] shadow-[8px_8px_24px_rgba(0,0,0,0.4),-4px_-4px_16px_rgba(255,255,255,0.03)] rounded-2xl border border-white/5";
  const neumorphicCardLight =
    "bg-gradient-to-br from-[#252d42] to-[#1a2030] shadow-[6px_6px_20px_rgba(0,0,0,0.35),-3px_-3px_12px_rgba(255,255,255,0.02)] rounded-xl border border-white/5";
  const neumorphicBtn =
    "px-5 py-2.5 rounded-xl font-bold transition-all active:scale-95 shadow-[4px_4px_12px_rgba(0,0,0,0.3),-2px_-2px_8px_rgba(255,255,255,0.02)] bg-gradient-to-br from-[#252d42] to-[#1a2030] text-gray-300 flex items-center gap-2 hover:text-amber-400 border border-white/5";
  const neumorphicBtnActive =
    "px-5 py-2.5 rounded-xl font-bold transition-all shadow-[inset_4px_4px_12px_rgba(0,0,0,0.4),inset_-2px_-2px_8px_rgba(255,255,255,0.02)] bg-gradient-to-br from-[#1a2030] to-[#151b28] text-amber-400 flex items-center gap-2 border border-amber-500/20";
  const neumorphicInset =
    "shadow-[inset_4px_4px_12px_rgba(0,0,0,0.4),inset_-2px_-2px_8px_rgba(255,255,255,0.02)] bg-gradient-to-br from-[#151b28] to-[#0d1117] rounded-xl border border-white/5";
  ...
  const clientAnalysis = useMemo((): ClientAnalysis[] => {
    ...
    sessions.forEach((session) => {
      ...
      if (!clientMap[normalizedClient].branches[branch]) {
        clientMap[normalizedClient].branches[branch] = {
          candidates: 0,
          sessions: 0,
        };
      }
      clientMap[normalizedClient].branches[branch].candidates +=
    ...
    Object.keys(clientMap).forEach((clientKey) => {
      const clientSessions = sessions.filter(
        (s) => normalizeClientName(s.client_name) === clientKey
      );
      const weeklyMap: {
        [key: number]: {
          candidates: number;
    ...
    Object.entries(dateMap).forEach(([date, daySessions]) => {
      ...
      if (hasOverlap) {
        ...
        if (excessCandidates > 0) {
          suggestions.push(
            `Consider early check-in for first exam (${
              overlapPeriods[0]?.sessions[0]?.exam_name || "first session"
            }) to clear ${Math.ceil(
              excessCandidates / 2
            )} candidates before overlap window.`
          );
        }

        // Check if extending beyond 5 PM would help
        const lastEndTime = Math.max(
          ...daySessions.map((s) => timeToMinutes(s.end_time))
        );
        if (lastEndTime <= timeToMinutes("17:00") && excessCandidates > 0) {
          suggestions.push(
            `Extend operating hours beyond 5 PM to accommodate ${excessCandidates} excess candidates.`
          );
        }
        ...
        if (overlapHours >= 1) {
          suggestions.push(
            `Stagger exam start times by ${Math.ceil(
              overlapHours
            )}+ hour(s) to reduce concurrent load.`
          );
        }
  ...
  const totals = useMemo(() => {
    const byBranch: {
      [branch: string]: { candidates: number; sessions: number };
    } = {};
    ...
    const weekMap: {
      [week: number]: { candidates: number; sessions: number };
    } = {};
  ...
  const exportAsCSV = () => {
    ...
    rows.push([
      "TOTALS",
      "",
      totals.totalCandidates.toString(),
      totals.totalSessions.toString(),
      ...totals.weeklyTotals.map((w) => w.candidates.toString()),
    ]);
...
`;
    ...
    clientAnalysis.forEach((client) => {
      report += `
┌─────────────────────────────────────────────────────────────────┐
│  ${client.clientName.padEnd(20)} Total: ${
        client.totalCandidates
      } Candidates (${client.sessionCount} Sessions)
├─────────────────────────────────────────────────────────────────┤
│  BY CENTER:
`;
      Object.entries(client.branches).forEach(([branch, data]) => {
        report += `│    ${branch.toUpperCase().padEnd(15)} │ ${data.candidates
          .toString()
          .padStart(4)} Candidates │ ${data.sessions} Sessions
`;
      });
...
`;
    ...
    Object.entries(totals.byBranch).forEach(([branch, data]) => {
      report += `
  ${branch.toUpperCase().padEnd(12)}: ${data.candidates
        .toString()
        .padStart(5)} Candidates | ${data.sessions} Sessions`;
    });
...
`;
    ...
    totals.weeklyTotals.forEach((week) => {
      report += `
  Week ${week.week}: ${week.candidates.toString().padStart(5)} Candidates | ${
        week.sessions
      } Sessions`;
    });
...
`;
      overlapAnalysis.forEach((overlap) => {
        const dateStr = new Date(overlap.date).toLocaleDateString("en-IN", {
          weekday: "short",
          day: "2-digit",
          month: "short",
        });
  ...
  ${dateStr}:
    ...
    • Total Candidates: ${overlap.totalCandidates} (Capacity: ${
          overlap.capacity
        })
      ...
      <div className="relative h-full flex flex-col max-w-[1800px] mx-auto px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                {/* Center Badge */}
                <div
                  className={`px-4 py-2 rounded-xl font-bold text-sm uppercase tracking-widest ${
                    isGlobalView
              ...
              <p className="text-gray-400 font-medium text-lg">
                {isGlobalView
                  ...
                  : `${
                      activeBranch.charAt(0).toUpperCase() +
                      activeBranch.slice(1)
                    } center analysis for invoice generation`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div
              className={`${neumorphicInset} px-3 py-2 flex items-center gap-3`}
            >
              <button
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 flex-1">
            ...
            <p className="text-gray-400 font-bold mt-6 animate-pulse">
              ...
              {selectedView === "overview" && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div
                      className={`${neumorphicCard} p-6 relative overflow-hidden group`}
                    >
                    ...
                    >
                      ...
                      <h3 className="text-rose-400 font-bold uppercase tracking-widest text-xs mb-2">
                        Overlap Alerts
                      </h3>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className={`${neumorphicCard} p-8`}>
                      ...
                      <div className="space-y-4">
                        {Object.entries(totals.byBranch).map(
                          ([branch, data]) => (
                            <div
                              key={branch}
                              className={`${neumorphicCardLight} p-5 flex items-center justify-between`}
                            >
                              <div className="flex items-center gap-4">
                    ...
                    <div className={`${neumorphicCard} p-8`}>
                      ...
                      <div className="space-y-4">
                        {totals.weeklyTotals.map((week) => (
                          <div
                            key={week.week}
                            className={`${neumorphicCardLight} p-5`}
                          >
                            ...
                            <div className="h-2 w-full bg-[#0d1117] rounded-full overflow-hidden">
                              <motion.div
                                ...
                                animate={{
                                  width: `${
                                    (week.candidates / totals.totalCandidates) *
                                    100
                                  }%`,
                                }}
                                transition={{
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-white/10">
                            <th className="text-left py-4 px-4 font-bold text-gray-500 text-xs uppercase tracking-wider">
                              Client
                            </th>
                            <th className="text-center py-4 px-4 font-bold text-gray-500 text-xs uppercase tracking-wider">
                              Total Candidates
                            </th>
                            <th className="text-center py-4 px-4 font-bold text-gray-500 text-xs uppercase tracking-wider">
                              Sessions
                            ...
                            {["Calicut", "Cochin", "Kannur"].map((branch) => (
                              <th
                                key={branch}
                                className="text-center py-4 px-4 font-bold text-gray-500 text-xs uppercase tracking-wider"
                              >
                                {branch}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {clientAnalysis.map((client) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="text-center py-4 px-4 text-gray-400 font-medium">
                                  {client.sessionCount}
                                </td>
                                {["calicut", "cochin", "kannur"].map(
                                  (branch) => (
                                    <td
                                      key={branch}
                                      className="text-center py-4 px-4 text-gray-400"
                                    >
                          ...
                          <tr className="border-t-2 border-amber-500/30 bg-amber-500/5">
                            ...
                            <td className="text-center py-4 px-4 text-amber-400 font-bold">
                              {totals.totalSessions}
                            </td>
                            {["calicut", "cochin", "kannur"].map((branch) => (
                              <td
                                key={branch}
                                className="text-center py-4 px-4 text-amber-400 font-bold"
                              >
                ...
                >
                  {clientAnalysis.map((client) => {
                    ...
                    return (
                      <div
                        key={client.clientName}
                        className={`${neumorphicCard} p-8 border-l-4 ${color.border}`}
                      >
                        <div className="flex items-center justify-between mb-6">
                          <div>
                            <h3 className={`text-2xl font-black ${color.text}`}>
                              {client.clientName}
                            </h3>
                            <p className="text-gray-500 text-sm">
                              {client.sessionCount} Total Sessions
                            </p>
                          </div>
                          <div
                            className={`px-6 py-3 bg-gradient-to-br ${color.bg} rounded-2xl`}
                          >
                        ...
                        <div className="mb-6">
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            {Object.entries(client.branches).map(
                              ([branch, data]) => (
                                <div
                                  key={branch}
                                  className={`${neumorphicCardLight} p-4 text-center`}
                                >
                              ...
                              >
                                ...
                                <div className="flex-1 h-3 bg-[#0d1117] rounded-full overflow-hidden">
                                  <motion.div
                                    ...
                                    style={{
                                      background: `linear-gradient(to right, ${color.text
                                        .replace("text-", "")
                                        .replace("-400", "")}, ${color.text
                                        .replace("text-", "")
                                        .replace("-400", "-600")})`,
                                    }}
                                  />
              ...
              {selectedView === "overlaps" && (
                ...
                >
                  {overlapAnalysis.length === 0 ? (
                    <div
                      className={`${neumorphicCard} p-16 flex flex-col items-center justify-center`}
                    >
                      ...
                      <p className="text-gray-500 text-center max-w-md">
                        All scheduled sessions are within capacity limits with
                    ...
                    <>
                        ...
                        <div className="flex items-center gap-4 text-rose-400">
                          ...
                          <div>
                            ...
                            <p className="text-gray-400 text-sm">
                              Review the following dates for scheduling
                              adjustments
                            </p>
                          </div>
                        </div>
                      ...
                      {overlapAnalysis.map((overlap, idx) => (
                        <div
                          key={idx}
                          className={`${neumorphicCard} p-8 border-l-4 border-rose-500/50`}
                        >
                          <div className="flex items-start justify-between mb-6">
                            <div>
                              <h3 className="text-xl font-black text-white">
                                {new Date(overlap.date).toLocaleDateString(
                                  "en-IN",
                                  {
                                    weekday: "long",
                                    day: "2-digit",
                              ...
                              <div className="flex items-center gap-4 mt-2">
                                ...
                                <span className="px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full text-xs font-bold">
                                  {overlap.totalCandidates} / {overlap.capacity}{" "}
                                  capacity
                                </span>
                            ...
                            <div className="space-y-3">
                              {overlap.sessions.map((session, sIdx) => {
                                ...
                                return (
                                  <div
                                    key={sIdx}
                                    className={`${neumorphicCardLight} p-4 flex items-center justify-between`}
                                  >
                                    <div className="flex items-center gap-4">
                                      <div
                                        className={`w-1 h-12 rounded-full ${color.border.replace(
                                          "border-",
                                          "bg-"
                                        )}`}
                                      />
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {overlap.suggestions.map((suggestion, sIdx) => (
                                <div
                                  key={sIdx}
                                  className="flex items-start gap-3 p-4 rounded-xl bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20"
                                >
                                  <Timer
                                    size={16}
