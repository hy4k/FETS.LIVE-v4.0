...
export const CandidateAnalysis: React.FC<CandidateAnalysisProps> = ({
  ...
  const neumorphicCard =
    "bg-gradient-to-br from-[#0f3535] to-[#082424] shadow-[8px_8px_24px_rgba(0,0,0,0.5),-4px_-4px_16px_rgba(50,150,150,0.02)] rounded-2xl border border-teal-500/10";
  const neumorphicCardLight =
    "bg-gradient-to-br from-[#134040] to-[#0a2a2a] shadow-[6px_6px_20px_rgba(0,0,0,0.4),-3px_-3px_12px_rgba(50,150,150,0.02)] rounded-xl border border-teal-500/10";
  const neumorphicBtn =
    "px-5 py-2.5 rounded-xl font-bold transition-all active:scale-95 shadow-[4px_4px_12px_rgba(0,0,0,0.4),-2px_-2px_8px_rgba(50,150,150,0.02)] bg-gradient-to-br from-[#134040] to-[#0a2a2a] text-gray-300 flex items-center gap-2 hover:text-teal-400 border border-teal-500/10";
  const neumorphicBtnActive =
    "px-5 py-2.5 rounded-xl font-bold transition-all shadow-[inset_4px_4px_12px_rgba(0,0,0,0.5),inset_-2px_-2px_8px_rgba(50,150,150,0.02)] bg-gradient-to-br from-[#0a2a2a] to-[#051a1a] text-teal-400 flex items-center gap-2 border border-teal-500/20";
  const neumorphicInset =
    "shadow-[inset_4px_4px_12px_rgba(0,0,0,0.5),inset_-2px_-2px_8px_rgba(50,150,150,0.02)] bg-gradient-to-br from-[#082424] to-[#051a1a] rounded-xl border border-teal-500/10";
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: candidateData, error: candidateError } =
        await candidateQuery.order("created_at", { ascending: false });
  ...
  const discrepancies = useMemo((): DiscrepancyInfo[] => {
    const calendarMap: Record<
      string,
      { client: string; exam: string; count: number }
    > = {};

    // Group calendar sessions by client + exam
    calendarSessions.forEach((session) => {
      const key = `${normalizeClientName(session.client_name)}_${
        session.exam_name || "General"
      }`;
      if (!calendarMap[key]) {
        calendarMap[key] = {
    ...
    monthCandidates.forEach((c) => {
      const key = `${normalizeClientName(c.client_name)}_${
        c.exam_name || "General"
      }`;
    ...
    allKeys.forEach((key) => {
      ...
      if (calendarEntry || registerCount > 0) {
        result.push({
          ...
          difference,
          status:
            difference === 0 ? "match" : difference > 0 ? "excess" : "shortage",
        });
      }
    });
...
`;
    ...
    Object.entries(metrics.examStats)
      ...
      .forEach(([exam, data]) => {
        report += `
  ${exam.substring(0, 30).padEnd(30)}: ${data.count.toString().padStart(5)} (${
          data.client
        })`;
      });
...
`;
    ...
    Object.entries(metrics.locationStats)
      ...
      .forEach(([location, count]) => {
        report += `
  ${location.substring(0, 25).padEnd(25)}: ${count
          .toString()
          .padStart(5)} Candidates`;
      });
...
`;
  ...
  ${d.clientName} - ${d.examName}
    Calendar: ${d.calendarCount} | Register: ${d.registerCount} | Diff: ${
            d.difference > 0 ? "+" : ""
          }${d.difference} ${status}`;
        });
    }
...
`;
    ...
    a.href = url;
    a.download = `FETS_Register_Analysis_${
      isGlobalView ? "Global" : activeBranch
    }_${format(currentMonth, "MMM_yyyy")}.txt`;
    a.click();
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-gray-400 font-medium">
                Complete analysis from beginning • Comparison month:{" "}
                      ...
                      {Object.entries(metrics.clientStats)
                        ...
                        .map(([client, count]) => {
                          ...
                          return (
                            <div
                              key={client}
                              className={`${neumorphicCardLight} p-4`}
                            >
                      ...
                      {Object.entries(metrics.examStats)
                        ...
                        .map(([exam, data], idx) => (
                          ...
                          >
                            ...
                            <span className="text-lg font-black text-teal-400 ml-4">
                              {data.count}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="space-y-4">
                    {Object.entries(metrics.staffStats)
                      ...
                      .map(([name, count], idx) => (
                        <div
                          key={name}
                          className={`${neumorphicCardLight} p-5 flex items-center gap-6`}
                        >
                          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 flex items-center justify-center relative">
                            ...
                            {idx < 3 && (
                              <div
                                className={`absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border-2 border-[#0a2a2a] ${
                                  idx === 0
                          ...
                          <div className="flex-1">
                            <p className="font-black text-white uppercase text-sm tracking-wider">
                              {name}
                            </p>
            ...
            {selectedView === "discrepancy" && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className={`${neumorphicCard} p-6`}>
                    ...
                    <p className="text-4xl font-black text-rose-400">
                      {
                        discrepancies.filter((d) => d.status === "shortage")
                          .length
                      }
                    </p>
                  </div>
                  <div className={`${neumorphicCard} p-6`}>
                    ...
                    <p className="text-4xl font-black text-amber-400">
                      {
                        discrepancies.filter((d) => d.status === "excess")
                          .length
                      }
                    </p>
                  </div>
                ...
                <div className={`${neumorphicCard} p-8`}>
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-xl font-black text-white">
                          Calendar vs Register Comparison
                        </h3>
                        <p className="text-xs text-gray-500">
                          Comparing {format(currentMonth, "MMMM yyyy")} data
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {discrepancies.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16">
                      ...
                      <p className="text-gray-500 text-sm">
                        No calendar sessions or register entries found for this
                        month
                      </p>
                    </div>
                  ) : (
                ...
                {discrepancies.filter((d) => d.status === "shortage").length >
                  0 && (
                  <div
                    className={`${neumorphicCard} p-6 border-l-4 border-rose-500`}
                  >
                    <div className="flex items-center gap-4">
                      <AlertTriangle size={24} className="text-rose-400" />
                      <div>
                        ...
                        <p className="text-gray-400 text-sm">
                          ...
                          exam(s) show fewer registered candidates than
                          scheduled in the calendar. Please verify the register
