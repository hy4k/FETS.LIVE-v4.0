...
const Message: React.FC<MessageProps> = ({ message }) => {
  ...
  useEffect(() => {
    if (inView && !isYou) {
      const isRead = read_receipts.some(
  ...
  const isReadByAll =
    read_receipts && message.conversation?.members
  ...
  return (
    ...
    >
      <img
        src={
          author.avatar_url ||
          `https://ui-avatars.com/api/?name=${author.full_name}&background=random`
        }
      ...
      <div className={`flex flex-col gap-1 ${isYou ? "items-end" : ""}`}>
        ...
        <div
          className={`p-3 rounded-lg ${
            isYou ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
        >
          <p>{content}</p>
        </div>
