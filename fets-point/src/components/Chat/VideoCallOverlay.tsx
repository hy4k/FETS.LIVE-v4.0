      ...
      {!isVideoRenderable && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0f172a] gap-4">
          ...
          <div className="flex flex-col items-center gap-1 z-10">
            ...
            <span className="text-emerald-400/60 text-[8px] font-bold uppercase tracking-widest">
              Voice Uplink Only
            </span>
          </div>
        </div>
        ...
        <div className="px-3 py-1.5 bg-black/60 backdrop-blur-md rounded-lg text-[10px] font-black text-white uppercase tracking-wider flex items-center gap-3 border border-white/10">
          <div
            className={`w-1.5 h-1.5 rounded-full ${
              isVideoRenderable ? "bg-amber-400" : "bg-emerald-400"
            }`}
          />
          {userName}
          {callType === "audio" && (
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          <div className="relative">
            <div
              className={`w-3 h-3 rounded-full ${
                callType === "audio" ? "bg-emerald-500" : "bg-[#ffbf00]"
              } animate-pulse`}
            />
            <div
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em] leading-none mb-1">
              {callType === "audio"
                ? "Voice Uplink Active"
                : "Mesh Video Link Active"}
            </span>
            <div className="flex items-center gap-2">
              <Clock size={12} className="text-white/20" />
              <span className="text-xs font-black text-white font-mono tracking-wider tabular-nums">
                {duration}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          {onToggleMinimize && (
            <button
              ...
              className="p-3 rounded-2xl bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-all border border-white/5"
            >
      ...
      >
        ...
        {participantsCount === 0 && (
          <div className="col-span-full h-full flex flex-col items-center justify-center text-white/10 border-2 border-dashed border-white/5 rounded-3xl">
            ...
            <p className="font-black text-xs uppercase tracking-[0.5em] opacity-40">
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center p-2 bg-slate-950/40 backdrop-blur-2xl rounded-3xl border border-white/10 z-[70] shadow-2xl">
          <div className="flex items-center gap-2 px-4">
            ...
            >
              <PhoneOff
                size={32}
                className="group-hover:rotate-[135deg] transition-transform duration-500"
              />
            </button>
