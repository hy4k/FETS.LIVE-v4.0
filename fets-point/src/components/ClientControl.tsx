...
export function ClientControl() {
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        .from("clients")
        .insert([{ name: newClientName.toUpperCase(), color: newClientColor }]);
  ...
  const activeClientExams = exams.filter(
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div
          ...
          style={{ borderColor: THEME.bgTertiary, borderTopColor: THEME.gold }}
        />
      </div>
    );
  }
  ...
  return (
    ...
    >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color: THEME.textMuted }}>
                      {exams.filter((e) => e.client_id === client.id).length}{" "}
                      exams
                    </p>
                  </div>
        ...
        {activeClient ? (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p
                  ...
                  style={{ color: THEME.textSecondary }}
                >
                  {activeClientExams.length} exam
                  {activeClientExams.length !== 1 ? "s" : ""} configured
                </p>
              </div>
              <motion.button
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <p className="text-sm mt-2" style={{ color: THEME.textMuted }}>
              Choose from the list to manage exams
            </p>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          color: style.text,
                          border:
                            editingClient.color === color.value
                              ? `2px solid ${THEME.gold}`
                              : "2px solid transparent",
                        }}
                      >
        ...
        {showAddExam && (
          <Modal
            title={`Add Exam to ${activeClient?.name}`}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {BRANCHES.map((branch) => (
                    <button
                      ...
                      style={{
                        background: newExam.locations.includes(branch)
                          ? THEME.goldMuted
                          : THEME.bgTertiary,
                        color: newExam.locations.includes(branch)
                          ? THEME.gold
                          : THEME.textSecondary,
                        border: newExam.locations.includes(branch)
                          ? `1px solid ${THEME.gold}40`
                    ...
                    >
                      <div
                        ...
                        style={{
                          borderColor: newExam.locations.includes(branch)
                            ? THEME.gold
                            : THEME.textMuted,
                          background: newExam.locations.includes(branch)
                            ? THEME.gold
                      ...
                      >
                        {newExam.locations.includes(branch) && (
                          <div
...
export async function getClientsAndExams(): Promise<{
  clients: Client[];
  exams: ClientExam[];
}> {
