      ...
      for (const staffId of selectedStaffIds) {
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
          ...
          newSchedules.push({
            ...
            shift_code: code,
            created_by: user?.id,
            branch_id: staffProfiles.find((s) => s.id === staffId)
              ?.branch_assigned,
          });
        }
      }
        ...
        <div className="px-6 py-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-1">
              Generate 6-1 roster pattern for{" "}
          ...
          <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-4 border border-blue-100 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-indigo-700 mt-1">
                Staff will work <span className="font-bold">{workShift}</span>{" "}
                shifts for 6 days, followed by 1{" "}
                <span className="font-bold">Rest Day</span>. Pattern repeats
                weekly starting from{" "}
                {new Date(patternStartDate).toLocaleDateString()}.
              </p>
            </div>
          </div>
