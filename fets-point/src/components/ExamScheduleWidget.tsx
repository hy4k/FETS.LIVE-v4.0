    ...
    if (activeBranch !== "global") {
      return sessions.map((session) => {
        ...
        return (
          <div
            key={session.id}
            className={`p-3 rounded-xl border-l-4 ${CLIENT_COLORS[clientType]} bg-white shadow-sm hover:shadow-md transition-all`}
          >
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[10px] text-gray-500 font-medium mb-1 uppercase bg-gray-100 w-fit px-1.5 rounded">
                  {session.exam_name}
                </p>
        ...
        <div className="grid grid-cols-1 gap-2">
          {bSessions.map((session) => {
            ...
            return (
              <div
                key={session.id}
                className={`p-3 rounded-xl border-l-4 ${CLIENT_COLORS[clientType]} bg-white shadow-sm border border-slate-100`}
              >
                <div className="flex justify-between items-start">
                  <div>
