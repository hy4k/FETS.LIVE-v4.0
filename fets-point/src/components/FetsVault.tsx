      ...
      const { error } = await supabase.from("fets_vault").insert([
        {
          ...
          tags: Array.isArray(newEntry.tags)
            ? newEntry.tags
            : ((newEntry.tags as string) || "")
                .split(",")
                .map((t: string) => t.trim())
                .filter(Boolean),
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a09] text-amber-100/80 rounded-3xl overflow-hidden border border-amber-900/30 shadow-2xl relative font-sans">
      ...
      <div className="relative z-10 p-6 flex flex-col gap-6 ">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`h-16 w-16 rounded-2xl border transition-all flex items-center justify-center ${
                isLocked
                  ? "bg-amber-600/20 text-amber-500 border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.3)]"
                  : "bg-[#1c1917] text-amber-800 border-amber-900/30 hover:border-amber-500/50 hover:text-amber-500"
              }`}
            >
            ...
            >
              ...
              <Plus
                size={32}
                className="text-amber-950 font-black relative z-10 drop-shadow-sm"
                strokeWidth={4}
              />
            </motion.button>
          </div>
        ...
        <div
          className={`relative group max-w-2xl mx-auto w-full transition-opacity duration-300 ${
            isLocked ? "opacity-0 pointer-events-none" : "opacity-100"
          }`}
        >
          <Search
            size={18}
            className="absolute left-5 top-1/2 -translate-y-1/2 text-amber-700 group-focus-within:text-amber-400 transition-colors"
          />
          <input
            type="text"
            value={searchTerm}
      ...
      {isLocked ? (
        <div className="absolute inset-0 z-20 bg-[#0a0a09] flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
          ...
          <ShieldCheck
            size={80}
            className="text-amber-600 mb-6 drop-shadow-[0_0_30px_rgba(245,158,11,0.3)] animate-pulse"
          />
          <h2 className="text-4xl font-black text-white uppercase tracking-widest mb-2">
            Vault Secured
          </h2>
          <p className="text-amber-700 font-bold tracking-wider mb-8 uppercase text-xs">
                      ...
                      <div>
                        <h3
                          className={`text-lg font-black uppercase tracking-wide ${
                            activeEntryId === entry.id
                              ? "text-amber-100"
                              : "text-amber-700 group-hover:text-amber-200"
                          }`}
                        >
                          {entry.title}
                        </h3>
                        <p className="text-[9px] uppercase tracking-widest text-amber-900/60 mt-1 font-bold">
                          {entry.category}
                        </p>
                      </div>
                    </div>
                        ...
                        <div className="pt-6 border-t border-amber-900/20 grid gap-6 grid-cols-1 md:grid-cols-2">
                            ...
                            {(entry.username || entry.password) && (
                              <div className="bg-black/40 p-4 rounded-xl border border-amber-900/30 space-y-3 shadow-inner">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-2 flex items-center gap-2">
                                  <Key size={10} /> Access Keys
                                </h4>

                                {entry.username && (
                                  <div className="flex justify-between items-center group/field">
                                    <div className="flex-1">
                                      <label className="text-[9px] uppercase tracking-widest text-amber-900 block">
                                        User Identity
                                      </label>
                                    ...
                                    <button
                                      ...
                                      className="text-amber-800 hover:text-amber-400"
                                    >
                                      <Copy size={14} />
                                    </button>
                                  </div>
                            ...
                            {entry.custom_fields &&
                              ...
                              entry.custom_fields.length > 0 && (
                                <div className="bg-black/40 p-4 rounded-xl border border-amber-900/30 space-y-2 shadow-inner">
                                  <h4 className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-2 flex items-center gap-2">
                                    <Database size={10} /> Custom Attributes
                                  </h4>
                                  {entry.custom_fields.map(
                                    (field: any, idx: number) => (
                                      <div
                                        key={idx}
                                        className="flex justify-between items-center pb-2 border-b border-amber-900/20 last:border-0 last:pb-0"
                                      >
                                        <div>
                                          <label className="text-[9px] uppercase tracking-widest text-amber-900 block">
                                            {field.key}
                          ...
                          <div className="space-y-4">
                            ...
                            {(entry.prof_email || entry.contact_numbers) && (
                              <div className="bg-black/40 p-4 rounded-xl border border-amber-900/30 space-y-3 shadow-inner">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-2 flex items-center gap-2">
                                  <Briefcase size={10} /> Contact Intel
                                </h4>
                                ...
                                {entry.contact_numbers && (
                                  <div className="pt-2 border-t border-amber-900/20">
                                    ...
                                    <div className="text-amber-100 text-sm whitespace-pre-line">
                                      {entry.contact_numbers}
                                    </div>
                                  </div>
                                )}
                              ...
                              <div className="bg-black/40 p-4 rounded-xl border border-amber-900/30 space-y-3 shadow-inner">
                                {entry.url && (
                                  <a
                                    ...
                                    className="flex items-center gap-2 text-xs font-black text-amber-400 hover:text-amber-300 uppercase tracking-wider group/link"
                                  >
                                    <Globe size={14} /> Launch Portal{" "}
                                    <ExternalLink
                                      size={10}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md overflow-y-auto">
            ...
            >
              ...
              <form
                ...
                className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar relative z-10"
              >
                {/* Section: Basic Info */}
                <div className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-widest text-amber-600 border-b border-amber-900/30 pb-2">
                    01. Asset Identification
                  </h4>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold text-amber-800 uppercase tracking-widest">
                        Name / Title
                      </label>
                      <input
                        autoFocus
                        ...
                        className="w-full bg-black/30 border border-amber-900/30 rounded-xl py-3 px-4 text-sm text-amber-100 focus:border-amber-500/50 outline-none transition-all focus:shadow-[0_0_10px_rgba(245,158,11,0.1)]"
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-black/30 border border-amber-900/30 rounded-xl py-3 px-4 text-sm text-amber-100 focus:border-amber-500/50 outline-none"
                        placeholder="General"
                      />
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center border-b border-amber-900/30 pb-2">
                    <h4 className="text-xs font-black uppercase tracking-widest text-amber-600">
                      02. Secure Keys
                    </h4>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-black/30 border border-amber-900/30 rounded-xl py-3 px-4 text-sm text-blue-300 focus:border-amber-500/50 outline-none"
                      placeholder="https://"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-black/30 border border-amber-900/30 rounded-xl py-3 px-4 text-sm text-amber-100 focus:border-amber-500/50 outline-none font-mono"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold text-amber-800 uppercase tracking-widest">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center border-b border-amber-900/30 pb-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1 rounded bg-amber-500/10 text-[10px] font-bold uppercase tracking-wider text-amber-500 hover:bg-amber-500 hover:text-black transition-colors flex items-center gap-2"
                    >
                  ...
                  {customFieldsInput.map((field, idx) => (
                    <div
                      key={idx}
                      className="flex gap-2 items-start animate-in fade-in slide-in-from-left-4"
                    >
                      <input
                        type="text"
                        ...
                        className="w-1/3 bg-black/30 border border-amber-900/30 rounded-xl py-2 px-3 text-xs text-amber-700 focus:border-amber-500/50 outline-none"
                      />
                      <input
                        type="text"
                        placeholder="Data"
                        ...
                        className="flex-1 bg-black/30 border border-amber-900/30 rounded-xl py-2 px-3 text-xs text-amber-100 focus:border-amber-500/50 outline-none"
                      />
                      <button
                        type="button"
                  ...
                  {customFieldsInput.length === 0 && (
                    <p className="text-[10px] text-amber-900/50 italic py-2">
                      No custom data points added.
                    </p>
                  )}
                </div>
                ...
                <div className="pt-6 border-t border-amber-900/30 flex gap-4">
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="flex-1 py-4 text-amber-800 font-bold uppercase tracking-widest text-xs hover:bg-amber-900/10 rounded-xl transition-colors"
                  >
                    Cancel
                  </button>
                  <button
