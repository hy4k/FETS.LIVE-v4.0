...
export const Frame = () => {
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      const members = [...new Set([profile.id, ...selectedMembers])].map(
        (uid) => ({
          group_id: grp.id,
          user_id: uid,
  ...
  const handleStartCall = (groupId: string) => {
    ...
    supabase
      ...
      .then(({ data }) => {
        const ids =
          data?.map((m) => m.user_id).filter((id) => id !== profile.id) || [];
        if (ids.length) {
  ...
  const handleScheduleMeeting = async () => {
    if (!meetingTitle || !meetingDate || !meetingTime || !groups.length)
    ...
    try {
      await supabase.from("scheduled_meetings").insert({
        ...
        room_url: `https://fets.live/meet/${Math.random()
          .toString(36)
          .substring(7)}`,
      });
      ...
      <div className="relative z-10 w-full h-full p-8 flex gap-8">
        ...
        <GlassPanel className="w-28 flex flex-col items-center py-8 gap-4 shrink-0">
          ...
          <button
            ...
            className="group w-16 h-16 rounded-[24px] bg-white text-black flex items-center justify-center hover:scale-105 transition-transform shadow-[0_0_20px_rgba(255,255,255,0.3)]"
          >
            <Plus size={28} />
          </button>
          ...
          <div className="flex items-center justify-between shrink-0 px-4">
            ...
            {activeTab === "schedule" && (
              <button
                ...
                className="px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/10 rounded-full text-sm font-medium transition-all backdrop-blur-md"
              >
                + Schedule Event
              </button>
            )}
              ...
              {activeTab === "calls" ? (
                ...
                >
                  {groups.map((group) => (
                    <GlassPanel
                      key={group.id}
                      className="p-6 group cursor-pointer hover:bg-white/10 transition-colors duration-300"
                    >
                      <div className="flex justify-between items-start mb-6">
                        ...
                        <div
                          onClick={(e) => {
                            e.stopPropagation();
                            handleStartCall(group.id);
                          }}
                          className="p-3 rounded-full bg-green-500/20 text-green-400 hover:bg-green-500 hover:text-white transition-all border border-green-500/30"
                        >
                          <Video size={20} fill="currentColor" />
                        </div>
                      </div>
                      <h3 className="text-xl font-medium text-white mb-1 group-hover:text-purple-300 transition-colors">
                        {group.name}
                      </h3>
                      <p className="text-sm text-white/40 line-clamp-1 mb-4">
                        {group.description || "Secure Channel"}
                      </p>
                      ...
                      <div className="flex items-center gap-2 mt-auto pt-4 border-t border-white/5">
                        <div className="flex -space-x-2">
                          {[1, 2, 3].map((i) => (
                            <div
                              key={i}
                              className="w-6 h-6 rounded-full bg-white/10 border border-black backdrop-blur-sm"
                            />
                          ))}
                        </div>
                  ...
                  <button
                    onClick={() => setShowNewGroup(true)}
                    className="group relative rounded-[32px] border border-dashed border-white/20 hover:border-white/40 hover:bg-white/5 transition-all flex flex-col items-center justify-center gap-4 min-h-[200px]"
                  >
                      ...
                      <GlassPanel className="ml-16 p-6 flex items-center justify-between hover:bg-white/10 transition-colors">
                        <div className="flex gap-6 items-center">
                          <div className="flex flex-col items-center justify-center w-16 h-16 rounded-2xl bg-white/5 border border-white/10">
                            <span className="text-xs uppercase text-white/40 font-bold">
                ...
                {showNewGroup ? (
                  <>
                    ...
                    <button
                      ...
                      className="w-full py-4 mt-2 bg-white text-black rounded-2xl font-medium hover:shadow-[0_0_30px_rgba(255,255,255,0.3)] transition-all"
                    >
                      Initialize Group
                    </button>
                  </>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <input
                        ...
                        className="bg-black/20 border border-white/10 rounded-2xl px-4 py-4 text-white/80 focus:outline-none"
                      />
                      <input
                        type="time"
                        value={meetingTime}
                    ...
                    <button
                      ...
                      className="w-full py-4 mt-4 bg-white text-black rounded-2xl font-medium hover:shadow-[0_0_30px_rgba(255,255,255,0.3)] transition-all"
                    >
                      Confirm Schedule
                    </button>
                  </>
