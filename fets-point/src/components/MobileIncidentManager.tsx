          ...
          <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 space-y-6">
            <div className="flex justify-between items-start">
              <div
                ...
                style={{
                  backgroundColor: getCategoryColor(incident.category).bgColor,
                  color: getCategoryColor(incident.category).color,
                }}
              >
      ...
      <div className="px-6 pt-12 pb-8 bg-white rounded-b-[45px] shadow-sm">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em] mt-2 px-1">
              Operations Log
            </p>
          </div>
          <button
      ...
      <div className="p-6 space-y-4">
        {loading ? (
          [1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-28 bg-white rounded-[35px] animate-pulse"
            />
          ))
        ) : filtered.length === 0 ? (
            ...
            >
              <div className="flex items-center gap-5">
                <div
                  ...
                  style={{
                    backgroundColor: getCategoryColor(inc.category).color,
                  }}
                >
                  <Activity size={24} className="text-white" />
                </div>
                <div>
                  <h3 className="font-black text-slate-900 uppercase tracking-tighter leading-tight mb-1 truncate max-w-[180px]">
                    {inc.title}
                  </h3>
                  <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">
                    CASE-{inc.id.slice(0, 6).toUpperCase()} • {inc.status}
                  </p>
                </div>
              </div>
