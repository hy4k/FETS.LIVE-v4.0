      ...
      <div className="px-6 pt-12 pb-6 bg-white border-b border-slate-100 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-xl font-black italic tracking-tighter text-slate-900 uppercase leading-none">
              MY DESK
            </span>
            <span className="text-[9px] font-black text-slate-300 uppercase tracking-[0.4em] mt-1">
              Live Grid
            </span>
          </div>
        </div>
            ...
            <div className="w-16 h-16 rounded-[22px] p-1 bg-gradient-to-tr from-amber-400 to-pink-500 flex items-center justify-center shadow-lg group-active:scale-95 transition-transform">
              <div className="w-full h-full rounded-[18px] border-[2px] border-white overflow-hidden bg-white">
                <img
                  src={
                    staff.avatar_url ||
                    `https://ui-avatars.com/api/?name=${staff.full_name}&background=random`
                  }
      ...
      <div className="flex flex-col p-4 space-y-4">
        {loading ? (
          [1, 2].map((i) => (
            <div
              key={i}
              className="h-96 bg-white rounded-[40px] animate-pulse shadow-sm"
            />
          ))
        ) : posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-10">
            ...
            <p className="text-sm text-slate-400 font-bold leading-relaxed">
              Broadcast an operational update to the team.
            </p>
          </div>
        ) : (
          posts.map((post, i) => (
              ...
              <div className="flex items-center justify-between px-6 py-6">
                ...
                >
                  ...
                  <div className="text-left">
                    <h3 className="text-xs font-black text-slate-900 tracking-widest uppercase leading-none">
              ...
              <div className="px-8 py-5 flex items-center justify-between border-t border-slate-50">
                <div className="flex items-center gap-10">
                  ...
                  >
                    <Heart
                      size={26}
                      className="text-slate-900 group-active:text-rose-500"
                    />
          ...
          >
            <div className="px-6 pt-12 pb-8 flex items-center justify-between border-b border-slate-50">
              ...
              <h3 className="text-xl font-black text-slate-900 uppercase italic tracking-tighter">
                {editingPost ? "Sync Update" : "New Broadcast"}
              </h3>
              <div className="w-10" />
            </div>
          ...
          >
            <div className="p-8 flex items-center justify-between sticky top-0 z-10 bg-slate-900/80 backdrop-blur-xl">
              <button
                ...
                className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center active:bg-white/10 transition-colors"
              >
                <ChevronLeft size={24} />
              </button>
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white/40 italic">
                Personnel Dossier
              </span>
              <button
                onClick={() => setIsChatOpen(true)}
                className="w-12 h-12 rounded-2xl bg-amber-500 text-slate-900 flex items-center justify-center active:scale-90 transition-transform shadow-lg shadow-amber-500/20"
              >
            ...
            <div className="px-8 pb-20 space-y-12">
              ...
              <div className="flex flex-col items-center text-center gap-6">
                <div className="relative w-40 h-40">
                  ...
                  <div className="w-full h-full rounded-full overflow-hidden border-8 border-white/5 shadow-2xl relative z-10">
                    <img
                      src={
                        selectedStaffProfile.avatar_url ||
                        `https://ui-avatars.com/api/?name=${selectedStaffProfile.full_name}&background=f59e0b&color=000`
                      }
                      className="w-full h-full object-cover"
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-3 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 inline-block">
                    <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">
                      {selectedStaffProfile.role?.replace("_", " ")}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                {[
                  {
                    label: "Unit/Branch",
                    value: selectedStaffProfile.branch_assigned || "GlobalHQ",
                    icon: Globe,
                  },
                  {
                    label: "Department",
                    value: selectedStaffProfile.department || "Operations",
                    icon: Grid,
                  },
                  {
                    label: "Induction",
                    value: selectedStaffProfile.created_at
                      ? format(
                          new Date(selectedStaffProfile.created_at),
                          "MMMM yyyy"
                        )
                      : "N/A",
                    icon: Zap,
                  ...
                  <div
                    key={item.label}
                    className="p-6 rounded-[35px] bg-white/5 border border-white/5 flex items-center gap-5"
                  >
                    <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-amber-500">
                      <item.icon size={20} />
                    </div>
                    <div className="text-left">
                      <p className="text-[9px] font-black text-white/30 uppercase tracking-widest leading-none mb-1.5">
                        {item.label}
                      </p>
              ...
              <div className="p-8 rounded-[45px] bg-gradient-to-br from-white/10 to-transparent border border-white/10 relative overflow-hidden">
                ...
                <p className="text-lg font-medium text-white/80 leading-relaxed italic">
                  "
                  {selectedStaffProfile.bio ||
                    "Personnel has not established a custom bio yet. Grid records remain at default clearance."}
                  "
                </p>
              </div>
            </div>
        ...
        {isChatOpen && selectedStaffProfile && (
          ...
          >
            ...
            <div className="px-6 pt-12 pb-6 border-b border-slate-50 flex items-center justify-between sticky top-0 bg-white z-10">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center active:scale-90 transition-transform"
                >
                  <ChevronLeft size={24} />
                </button>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl overflow-hidden shadow-sm">
                    <img
                      src={
                        selectedStaffProfile.avatar_url ||
                        `https://ui-avatars.com/api/?name=${selectedStaffProfile.full_name}&background=random`
                      }
                      className="w-full h-full object-cover"
                    />
                  </div>
                  <div className="text-left">
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-tighter leading-none mb-1">
                      {selectedStaffProfile.full_name.split(" ")[0]}
                    </h3>
            ...
            <div className="flex-1 bg-slate-50 p-6 flex flex-col items-center justify-center text-center gap-6 overflow-y-auto no-scrollbar">
              ...
              <div>
                <h4 className="font-black text-slate-900 uppercase tracking-tight text-xl mb-2 italic">
                  End-to-End Encrypted
                </h4>
                <p className="text-sm text-slate-400 font-medium leading-relaxed max-w-[240px]">
                  This channel is secured with FETS RSA-2048 protocol.
                </p>
              </div>
              <div className="w-full mt-10">
                <div className="flex justify-end mb-4">
                  <div className="bg-[#3E2723] text-amber-500 p-5 rounded-[28px] rounded-tr-none text-sm font-bold shadow-lg max-w-[80%]">
                    Hey {selectedStaffProfile.full_name.split(" ")[0]}, can you
                    confirm the status for the morning shift?
                  </div>
                </div>
