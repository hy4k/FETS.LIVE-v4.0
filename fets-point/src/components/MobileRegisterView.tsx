    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] pb-32 pt-safe">
      ...
      <div className="bg-white px-6 pt-12 pb-8 border-b border-slate-100 shadow-sm sticky top-0 z-50 rounded-b-[40px]">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tighter leading-none uppercase italic">
              Register
            </h1>
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mt-1.5">
              Candidate Database
            </p>
          </div>
          ...
          <AnimatePresence mode="popLayout">
            {loading
              ? [1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="w-full h-32 bg-white rounded-[40px] animate-pulse shadow-sm"
                  ></div>
                ))
              : filtered.map((c, i) => (
                  <motion.div
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-16 h-16 rounded-full border-4 border-slate-50 overflow-hidden shadow-lg`}
                        >
                          <img
                            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(
                              c.full_name
                            )}&background=F6C845&color=4E342E&bold=true&size=128`}
                            className="w-full h-full object-cover"
                            alt="Avatar"
                          />
                        </div>
                        <div>
                          <h3 className="font-black text-slate-900 text-base leading-tight mb-1 truncate max-w-[180px]">
                            {c.full_name}
                          </h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-50">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] font-black text-slate-700 leading-none uppercase">
                            {c.branch_location}
                          </span>
                        </div>
                      </div>
