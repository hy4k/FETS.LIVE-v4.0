...
const getShiftStyle = (code: string) => {
  ...
  const base =
    "transition-all duration-300 transform hover:scale-105 font-bold font-rajdhani flex items-center justify-center rounded-xl bg-slate-50 shadow-[-2px_-2px_5px_rgba(255,255,255,1),2px_2px_5px_rgba(0,0,0,0.1)] border border-slate-100";
          ...
          <table className="min-w-full border-separate border-spacing-0">
              ...
              <tr>
                ...
                {days.map((d, idx) => {
                  ...
                  return (
                    <th
                      key={idx}
                      className={`relative z-10 border-b border-gray-100 px-2 py-3 min-w-[54px] text-center transition-colors hover:bg-yellow-50/50 ${
                        today ? "bg-yellow-50/80" : "bg-white/90"
                      }`}
                    >
                      <div className="flex flex-col items-center gap-1">
                        <span
                          className={`text-[10px] uppercase font-bold tracking-widest ${
                            today ? "text-yellow-700" : "text-gray-400"
                          }`}
                        >
                          {d
                      ...
                      {today && (
                        <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-yellow-400 mx-2 rounded-t-full"></div>
                      )}
                    </th>
                  );
                })}
            ...
            <tbody>
              {staffProfiles.map((staff, rIdx) => (
                <tr
                  key={staff.id}
                  className="group hover:bg-white/60 transition-colors duration-150"
                >
                  {/* Sticky Name Cell */}
                  <td className="sticky left-0 z-10 bg-white/95 group-hover:bg-yellow-50/20 backdrop-blur-sm border-b border-r border-gray-100 px-6 py-4 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div
                        className={`w-10 h-10 rounded-xl bg-slate-50 shadow-[-2px_-2px_5px_rgba(255,255,255,1),2px_2px_5px_rgba(0,0,0,0.1)] border border-slate-100 flex items-center justify-center font-bold shrink-0 text-base ${getAvatarColor(
                          staff.full_name
                        )}`}
                        style={rajdhaniStyle}
                      >
                      ...
                      <div className="flex flex-col min-w-0">
                        <span
                          className="text-gray-900 font-bold text-base truncate leading-tight"
                          style={rajdhaniStyle}
                        >
                          {staff.full_name}
                        </span>
                  ...
                  {days.map((d, cIdx) => {
                    ...
                    return (
                      <td
                        ...
                        className={`border-b border-gray-50 px-1 py-2 text-center align-middle cursor-pointer relative ${
                          today ? "bg-yellow-50/10" : ""
                        }`}
                      >
                        ...
                        <div className="w-full h-full flex items-center justify-center p-1">
                          {s ? (
                            <div
                              className={`relative w-10 h-10 ${getShiftStyle(
                                code
                              )}`}
                            >
                              ...
                              {(s.overtime_hours || 0) > 0 && (
                                <div
                                  className="absolute -top-1 -right-1 w-4 h-4 bg-purple-600/90 text-[8px] text-white flex items-center justify-center rounded-full border border-white shadow-sm font-bold z-10"
                                  title={`Overtime: ${s.overtime_hours} hrs`}
                                >
                                  OT
                                </div>
                              )}
