    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      ...
      >
        <div className="bg-white p-4 border-b border-slate-100 flex justify-between items-center">
          ...
          <button onClick={onClose} className="text-blue-500 font-bold text-sm">
            Cancel
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-0">
          ...
          <div className="px-4 pb-4">
            <div className="flex gap-2 overflow-x-auto no-scrollbar py-2">
              {(["General", "Appreciation", "Idea", "Fun"] as PulseType[]).map(
                (t) => (
                  <button
                    key={t}
                    type="button"
            ...
            <div className="w-8 h-8 rounded-full bg-slate-200 overflow-hidden">
              <img
                src={
                  ...
                  `https://ui-avatars.com/api/?name=${post.user?.full_name}`
                }
                className="w-full h-full object-cover"
              />
            </div>
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              >
                <Heart
                  size={24}
                  className={
                    isLiked ? "text-red-500 fill-current" : "text-slate-800"
                  }
                />
              </button>
              <button>
  ...
  return (
    ...
    >
      {post.image_url ? (
        ...
        <div
          className={`w-full h-full flex items-center justify-center p-4 text-center ${theme.text}`}
        >
          <span className="font-[Outfit] font-bold text-sm line-clamp-4">
            {post.content}
          </span>
        </div>
      )}
      ...
      <div className="flex flex-col items-center md:items-start flex-1 w-full">
        <div className="flex flex-col md:flex-row items-center gap-4 mb-4">
          <h2 className="text-2xl font-normal text-slate-800">
            {profile?.full_name || "Fetsian"}
          </h2>
          <div className="flex gap-2">
            <button className="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 rounded-lg text-sm font-bold text-slate-700 transition-colors">
              Edit Profile
            </button>
            <button className="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 rounded-lg text-sm font-bold text-slate-700 transition-colors">
              View Archive
        ...
        <div className="flex items-center justify-center md:justify-start gap-8 md:gap-12 mb-5 w-full text-slate-800">
          <div className="text-center md:text-left">
            <span className="font-bold block md:inline">124</span> posts
          </div>
          <div className="text-center md:text-left">
            <span className="font-bold block md:inline">452</span> followers
          </div>
          <div className="text-center md:text-left">
            <span className="font-bold block md:inline">318</span> following
          </div>
        </div>

        <div className="text-sm text-slate-700 text-center md:text-left space-y-1">
          <p className="font-bold">
            {profile?.role || "Fetsian"} @ {activeBranch}
          </p>
...
const WallSection = ({ profile }: { profile: any }) => {
  const certs = useMemo(() => {
    ...
    return Array.isArray(c)
      ? [...c, ...old.map((o: any) => ({ title: o, issuedBy: "Official" }))]
      : [];
  }, [profile]);
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        {certs.length > 0 ? (
          certs.map((item: any, idx: number) => (
            <div
              key={idx}
              className="bg-white p-8 border border-slate-100 shadow-xl text-center"
            >
...
export function MyDeskLivingBoard({
  onNavigate,
}: {
        ...
        <div className="border-t border-slate-200 mb-1">
          <div className="flex justify-center gap-12">
            <button
              ...
              className={`flex items-center gap-2 py-4 text-xs font-bold uppercase tracking-widest border-t border-transparent transition-all ${
                activeSection === "pulse"
