...
const NewsModal = ({ isOpen, onClose, newsItem, onSave }) => {
  const [formData, setFormData] = useState({
    ...
    expires_at: newsItem?.expires_at
      ? format(new Date(newsItem.expires_at), "yyyy-MM-dd")
      : "",
        ...
        <div className="p-6 border-b border-gray-200/50 bg-white/30 flex justify-between items-center">
          <div>
            <h3 className="text-xl font-black text-gray-800 uppercase tracking-tight">
              {newsItem ? "Edit Notice" : "New Notice"}
            </h3>
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-black/5 rounded-xl transition-colors text-gray-500"
          >
            <X size={20} />
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-gray-400 font-medium ml-1">
                Optional: Auto-remove after date
              </p>
            </div>
...
export function NewsManager() {
  ...
  const handleAiAnalysis = async () => {
    setIsAnalyzing(true);
    const activeNotices = newsItems
      .filter((n) => n.is_active)
      .map(
        (n) =>
          `- [${(n.priority || "normal").toUpperCase()}] (${
            n.branch_location || "global"
          }): ${n.content}`
      )
      .join("\n");
              ...
              <div className="flex-1 pl-4">
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  ...
                  {item.expires_at && (
                    <span className="flex items-center gap-1 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 px-2 py-1 rounded">
                      ...
                      {format(new Date(item.expires_at), "dd MMM")}
                    </span>
                  )}
                </div>
                ...
                <p
                  className={`text-lg font-medium leading-relaxed ${
                    item.is_active ? "text-gray-800" : "text-gray-400"
                  }`}
                >
                  {item.content}
                </p>
