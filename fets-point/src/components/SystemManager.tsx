...
const SystemIcon = ({
  ...
  if (type === "peripheral") {
    switch (itemType?.toLowerCase()) {
      case "printer":
        return <Printer style={{ color: color }} size={24} />;
      case "webcam":
        return <Monitor style={{ color: color }} size={24} />;
      case "headphones":
        return <Headphones style={{ color: color }} size={24} />;
      case "e sign pad":
        return <Keyboard style={{ color: color }} size={24} />;
      default:
        return <Package style={{ color: color }} size={24} />;
    }
  }
  if (type === "rented") return <Clock style={{ color: color }} size={24} />;
  switch (type) {
    case "server":
      return <Server style={{ color: color }} size={24} strokeWidth={2.5} />;
    case "admin":
      return <Shield style={{ color }} size={24} strokeWidth={2.5} />;
    default:
      return <Monitor style={{ color: color }} size={24} strokeWidth={2.5} />;
  }
};
...
const StatusBadge = ({ status, theme }: { status: string; theme: any }) => {
  ...
  return (
    <div
      style={config.style}
      className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border backdrop-blur-sm shadow-sm transition-all duration-300`}
    >
      <div
...
const GlassInset = ({
  ...
  <div
    className={`bg-white/20 backdrop-blur-md border border-white/20 rounded-2xl shadow-inner ${className}`}
  >
    {children}
  </div>
);
...
const SystemGridCard: React.FC<SystemGridCardProps> = ({
  ...
  const theme =
    branchThemes[activeBranch.toLowerCase()] || branchThemes["default"];
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`p-5 rounded-[2rem] border-4 cursor-pointer shadow-lg flex flex-col items-center text-center gap-2 transition-all h-full ${
          sizeClasses[variant]
        } ${
          isTransferredOut
      ...
      >
        ...
        <div
          style={
            !isTransferredOut && sys.status === "operational"
            ...
            <div className="mt-2 space-y-0.5">
              ...
              {sys.last_os_update && (
                <div className="text-[8px] font-bold text-blue-600 uppercase mt-1">
                  Last Update:{" "}
                  {format(new Date(sys.last_os_update), "dd MMM yy")}
                </div>
              )}
            </div>
          )}
        ...
        {isTransferredOut && (
          <div className="mt-1 px-2 py-0.5 bg-slate-200 rounded-full text-[7px] font-black text-slate-900 uppercase">
            TRANSFERRED
          </div>
        )}
        {isRental && (
          <div className="mt-1 px-2 py-0.5 bg-[#FFB6B9] rounded-full text-[7px] font-black text-black uppercase">
            RENTAL
          </div>
        )}
      </motion.div>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <span className="text-[10px] font-bold text-black uppercase block">
                      {sys.specs.cpu || "N/A"} - {sys.specs.ram || "N/A"}
                    </span>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    <span className="block text-[8px] font-black text-slate-400 uppercase mb-1">
                    ...
                    <span className="text-[10px] font-bold text-black uppercase block">
                      {sys.specs.os || "Windows 11"}
                    </span>
                  </div>
                  <div className="p-3 bg-blue-50/50 rounded-2xl border border-blue-100">
                    <span className="block text-[8px] font-black text-blue-400 uppercase mb-1">
                      Last Windows Update
                    </span>
              ...
              {isPeripheral && (
                <div className="p-4 bg-white rounded-2xl border-2 border-slate-100 flex justify-between items-center">
                  <span className="text-xs font-bold text-black uppercase">
                    {sys.it_item_type || "IT ITEM"}
                  </span>
                  <SystemIcon
                    type="peripheral"
                    itemType={sys.it_item_type}
                    color={theme.dark}
                  />
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditSystemOpen(sys);
                  }}
                  className="p-2.5 bg-slate-100 text-slate-900 rounded-xl hover:bg-slate-200 transition-all flex items-center justify-center gap-2 group border border-slate-200"
                >
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowTransferModal(sys);
                    setTargetBranch(sys.branch_location);
                  }}
                  className="p-2.5 bg-teal-50 text-teal-700 rounded-xl hover:bg-teal-100 transition-all flex items-center justify-center gap-2 col-span-2 border border-teal-100"
                >
              ...
              <button
                ...
                className="w-full py-2 hover:bg-rose-50 text-rose-500 rounded-xl text-[8px] font-black uppercase tracking-widest transition-all"
              >
...
const SystemManager = () => {
  ...
  const [showTransferModal, setShowTransferModal] = useState<System | null>(
    null
  );
  ...
  const [softEntry, setSoftEntry] = useState<Software>({
    name: "",
    install_date: "",
    client: "",
  });
  ...
  const handleCreateSystem = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      category_name:
        newSystem.category_name ||
        (newSystem.type === "peripheral" ? newSystem.it_item_type : "Systems"),
      name: newSystem.name,
    ...
    if ((newSystem as any).id) {
      ...
      if (!error && data?.[0]) {
        await logSystemAction(
          data[0].id,
          "creation",
          `System registered at ${activeBranch}`
        );
      }
    }
    ...
    if (error) {
      ...
      toast.success(
        (newSystem as any).id ? "System Updated" : "System Registered"
      );
  ...
  const handleDeleteSystem = async (systemId: string, name: string) => {
    if (
      !window.confirm(
        `Are you sure you want to permanently delete system ${name}?`
      )
    )
      return;
  ...
  const executeTransfer = async () => {
    ...
    try {
      ...
      await logSystemAction(
        ...
        "location_transfer",
        `Transferred from ${showTransferModal.branch_location} to ${targetBranch}`
      );
  ...
  const handleAutoIncident = async (system: System) => {
    if (!incidentDescription.trim())
      return toast.error("Provide incident details");
    ...
    try {
      ...
      await updateStatus(
        system.id,
        "fault",
        `Reported Issue: ${incidentDescription}`
      );
  ...
  const generateSerial = () => {
    ...
    setNewSystem((prev) => ({
      ...prev,
      specs: { ...prev.specs, serial_number: result },
    }));
  };
  ...
  const addSoftwareToDraft = (isNew: boolean, system?: System) => {
    if (!softEntry.name || !softEntry.client)
  ...
  const filteredSystems = systems
    .filter((sys) => {
      ...
      const matchesCategory =
        activeCategory === "all" || effectiveCategory === activeCategory;
      return (
        sys.branch_location === activeBranch && matchesCategory && matchesSearch
      );
    })
    .sort((a, b) =>
  ...
  return (
    <div
      style={{
        background: `linear-gradient(135deg, ${theme.bg}, ${theme.bg}, ${theme.secondary}30)`,
      }}
      className="min-h-screen text-[#2D3748] p-4 md:p-8 pt-6 font-['Plus_Jakarta_Sans',sans-serif]"
    >
      {/* Header Controls */}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div
                style={{ borderColor: `${theme.primary}20` }}
                className="flex items-center gap-3 px-4 py-2 bg-white/60 backdrop-blur-md border rounded-2xl shadow-sm"
              >
                <div className="relative">
                  ...
                  <div
                    style={{ backgroundColor: theme.primary }}
                    className="absolute inset-0 h-2.5 w-2.5 rounded-full animate-ping opacity-40"
                  />
                </div>
              ...
              />
              <span className="text-[10px] font-bold text-slate-800 uppercase tracking-[0.2em] opacity-60">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-black uppercase tracking-widest leading-none mb-1">
                  Current Status
                </span>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-72 flex flex-col gap-4">
            <GlassCard className="p-4 flex flex-col gap-2" glow>
              <span className="text-[10px] font-black text-black/40 uppercase tracking-[0.3em] ml-2 mb-2">
                Asset Directory
              </span>
              {categories.map((cat) => {
                const IconComp =
            ...
            <GlassCard className="p-4 space-y-4" glow>
              <div className="relative group">
                <Search
                  style={{ color: theme.primary }}
                  className="absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:scale-110 transition-transform"
                  size={16}
                />
                <input
                  type="text"
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-20 relative">
                    ...
                    <div className="flex flex-col items-center gap-8 w-full relative z-20">
                      <GlassCard
                        className="px-10 py-4 bg-slate-900 border-slate-700/50 shadow-2xl"
                        glow
                      >
                      ...
                      <div className="flex flex-wrap justify-center gap-12">
                        {systems
                          .filter(
                            (s) =>
                              s.system_type === "server" &&
                              s.branch_location === activeBranch
                          )
                          .map((sys) => (
                            <SystemGridCard
                              key={sys.id}
                        ...
                        {systems.filter(
                          ...
                          <div className="p-8 border-2 border-dashed border-slate-300 rounded-[2rem] opacity-40 text-[10px] uppercase font-black">
                            No Servers Deployed
                          </div>
                        )}
                      </div>
                    ...
                    <div className="absolute top-[340px] left-1/2 -translate-x-1/2 w-full h-20 -z-0 opacity-30 pointer-events-none">
                      ...
                      >
                        <path
                          d="M 500 0 L 500 50 M 200 50 L 800 50 M 200 50 L 200 100 M 500 50 L 500 100 M 800 50 L 800 100"
                          stroke={theme.primary}
                          strokeWidth="2"
                          fill="none"
                          strokeDasharray="5,5"
                ...
                >
                  ...
                  {systems.filter(
                    ...
                    <div className="col-span-full py-40 text-center border-4 border-dashed border-[#BBDED6] rounded-[3rem] bg-white/20">
                      <Package
                        size={64}
                        className="mx-auto mb-6 text-[#BBDED6]"
                        strokeWidth={1.5}
                      />
                      <p className="font-black text-black uppercase tracking-[0.25em] text-lg">
                        Inventory Empty
                      </p>
                      <p className="text-xs font-bold text-black uppercase tracking-widest mt-2 opacity-50">
                        No items registered in {activeCategory}
                      </p>
                    </div>
                  )}
                </motion.div>
            ...
            >
              <div className="flex justify-between items-center p-8 border-b border-[#BBDED6]/40 bg-white/40">
                <div>
                  ...
                  <p className="text-[10px] font-black text-black uppercase tracking-[0.3em] mt-1">
                    Move system to new location
                  </p>
                </div>
                <button
                  onClick={() => setShowTransferModal(null)}
                  className="p-3 text-black hover:text-[#2D3748] hover:bg-[#BBDED6]/30 rounded-2xl transition-all"
                >
              ...
              <div className="p-10 space-y-8">
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-black uppercase tracking-[0.2em] block ml-1">
                    Target Location
                  </label>
                  <div className="relative">
                    <select
                    ...
                    <ChevronDown
                      className="absolute right-5 top-1/2 -translate-y-1/2 text-[#61C0BF] pointer-events-none"
                      size={18}
                      strokeWidth={3}
                    />
                  </div>
                ...
                <div className="pt-4 flex flex-col gap-3">
                  <button
                    ...
                    disabled={
                      !targetBranch ||
                      loading ||
                      targetBranch === showTransferModal.branch_location
                    }
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
            ...
            >
              <div className="flex justify-between items-center p-8 border-b border-[#BBDED6]/40 bg-white/40">
                <div>
                  <h2 className="text-3xl font-black text-black uppercase tracking-tighter">
                    {(newSystem as any).id ? "Edit" : "Create"}{" "}
                    <span className="text-[#61C0BF]">System</span>
                  </h2>
                  <p className="text-xs font-black text-black uppercase tracking-[0.3em] mt-1">
              ...
              <form
                ...
                className="p-10 overflow-y-auto custom-scrollbar flex-1 bg-gradient-to-b from-white/40 to-transparent"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-8">
                  <div className="space-y-8">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-black uppercase tracking-[0.2em] ml-2">
                        System Name
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        value={newSystem.ip}
                        onChange={(e) =>
                          setNewSystem({ ...newSystem, ip: e.target.value })
                        }
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {["workstation", "server", "admin"].map((type) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setNewSystem({ ...newSystem, type: type as any })
                            }
                            className={`p-4 rounded-2xl flex flex-col items-center gap-2 border-2 transition-all duration-300 ${
                              newSystem.type === type
                      ...
                      <div className="space-y-6 p-6 bg-white border-2 border-[#BBDED6] rounded-[2rem] shadow-sm">
                        <div className="space-y-3">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setNewSystem({
                                ...newSystem,
                                it_item_type: e.target.value,
                                category_name:
                                  e.target.value === "Printer"
                                    ? "Printer"
                                    : [
                                        "Webcam",
                                        "Headphones",
                        ...
                        {newSystem.it_item_type === "Printer" && (
                          <div className="grid grid-cols-2 gap-4">
                            <input
                              placeholder="INK CAPACITY"
                              className="p-4 bg-[#FAE3D9]/20 border border-[#BBDED6] rounded-xl text-xs font-bold"
                            />
                            <input
                    ...
                    <div className="space-y-4 pt-8 border-t border-[#BBDED6]/40">
                      <label className="text-[10px] font-black text-black uppercase tracking-[0.2em] ml-2 block">
                        Support Access
                      </label>
                        ...
                        {clients.map((c) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-2 ${
                              newSystem.supported_clients.includes(c.name)
            ...
            >
              <div className="flex justify-between items-center p-10 border-b border-[#BBDED6]/40 bg-white/40">
                ...
                <button
                  ...
                  className="p-4 text-black hover:text-[#2D3748] hover:bg-[#BBDED6]/20 rounded-3xl transition-all"
                >
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-3">
                              <input
                                type="date"
                                defaultValue={
                                  showManageModal.last_os_update || ""
                                }
                                onChange={(e) =>
                                  setShowManageModal({
                                    ...showManageModal,
                              ...
                              <button
                                onClick={() =>
                                  updateStatus(
                                    ...
                                    `SYSTEM UPDATE: ${showManageModal.last_os_update}`
                                  )
                                }
                  ...
                  <div className="p-8 rounded-[2.5rem] bg-[#2D3748] border-2 border-[#61C0BF]/20 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="space-y-3 h-[250px] overflow-y-auto pr-4 custom-scrollbar relative z-10">
                      {systemLogs.map((log) => (
                        <div
                          key={log.id}
                          className="flex gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors group"
                        >
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              <span
                                className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ${
                                  log.log_type === "fault"
                                    ? "bg-[#FFB6B9]/20 text-[#FFB6B9]"
                                    : log.log_type === "status_change"
                                    ? "bg-[#61C0BF]/20 text-[#61C0BF]"
                      ...
                      {systemLogs.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center opacity-20">
                          ...
                          <p className="text-[10px] uppercase font-black tracking-[0.4em] text-[#61C0BF]">
                            Signal Void
                          </p>
                        </div>
                      )}
