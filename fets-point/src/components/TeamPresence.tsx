...
export function TeamPresence({ onFetsMeetClick }: TeamPresenceProps) {
  ...
  const onlineStaff = staff.filter(
    (s) => presence[s.user_id]?.status === "online"
  );
  const offlineStaff = staff.filter(
    (s) => !presence[s.user_id] || presence[s.user_id]?.status !== "online"
  );
  ...
  // Neumorphic styles
  const neuCard =
    "bg-[var(--dashboard-bg, #EEF2F9)] rounded-2xl shadow-[6px_6px_12px_var(--neu-dark-shadow,rgb(209,217,230)),-6px_-6px_12px_var(--neu-light-shadow,rgba(255,255,255,0.8))]";
  const neuBtn =
    ...
    <>
      ...
      >
          ...
          <div className="grid grid-cols-2 gap-3">
            {allStaff.map((agent) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className={`flex items-center gap-3 p-2 rounded-xl border border-white/50 cursor-pointer transition-all ${
                  openChats.find((c) => c.id === agent.id)
                    ? "bg-white shadow-sm"
                    : "hover:bg-white/60"
                }`}
                onClick={() =>
        ...
        <div className="pt-3 border-t border-slate-200/60 flex flex-col gap-2">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-black text-amber-600 uppercase tracking-widest bg-[#EEF2F9] px-2">
              FETS MEET
            </span>
            <div className="h-[1px] bg-slate-200 flex-1" />
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-slate-200 overflow-hidden">
                      <img
                        src={
                          s.avatar_url ||
                          `https://ui-avatars.com/api/?name=${s.full_name}`
                        }
                        className="w-full h-full object-cover"
                      />
                    </div>
      ...
      {openChats.map((targetUser, idx) => (
        <FetsChatPopup
          ...
          onClose={() =>
            setOpenChats((prev) => prev.filter((c) => c.id !== targetUser.id))
          }
          zIndex={2000 + idx}
        />
      ))}
