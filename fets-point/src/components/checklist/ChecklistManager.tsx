...
import {
  ChecklistTemplate,
  ChecklistSubmissionWithDetails as ChecklistSubmission,
...
export const ChecklistManager: React.FC<ChecklistManagerProps> = ({
  ...
  const [view, setView] = useState<"list" | "create" | "history" | "analysis">(
    "list"
  );
  ...
  const neumorphicInset =
    "bg-[#e0e5ec] shadow-[inset_6px_6px_10px_0_rgba(163,177,198,0.7),inset_-6px_-6px_10px_0_rgba(255,255,255,0.8)] rounded-xl border-none";
  const neumorphicBtn =
    "px-6 py-2.5 rounded-xl font-bold transition-all active:scale-95 active:shadow-[inset_4px_4px_8px_rgba(163,177,198,0.6),inset_-4px_-4px_8px_rgba(255,255,255,0.5)] shadow-[6px_6px_10px_rgba(163,177,198,0.6),-6px_-6px_10px_rgba(255,255,255,0.5)] bg-[#e0e5ec] text-gray-600 flex items-center gap-2 hover:text-blue-600";
  const neumorphicBtnActive =
    "px-6 py-2.5 rounded-xl font-bold transition-all shadow-[inset_4px_4px_8px_rgba(163,177,198,0.6),inset_-4px_-4px_8px_rgba(255,255,255,0.5)] bg-[#e0e5ec] text-amber-600 flex items-center gap-2 transform scale-105";
  const neumorphicIconBtn =
    "p-3 rounded-full transition-all active:scale-95 shadow-[5px_5px_10px_rgba(163,177,198,0.6),-5px_-5px_10px_rgba(255,255,255,0.5)] bg-[#e0e5ec] hover:text-blue-600";
    ...
    try {
      ...
      setHistory(
        (submissions || []).map((s: any) => ({
          ...s,
          submitted_by_profile: s.submitted_by_profile || {
  ...
  const deleteTemplate = async (id: string) => {
    if (
      !confirm(
        "Are you sure? This will delete the template and ALL associated history/submissions. This cannot be undone."
      )
    )
      return;
    ...
    try {
      ...
      toast.error(
        `Failed to delete: ${error.message || "It may have existing data"}`
      );
    }
  };
  ...
  if (view === "create") {
    return (
      <div
        className="min-h-screen -mt-32 pt-48 bg-[#e0e5ec] pb-12"
  ...
  const filteredTemplates = templates.filter((t) => {
    ...
    const matchesBranch =
      !t.branch_location ||
      t.branch_location === "global" ||
      t.branch_location === activeBranch;
  ...
  const historyFiltered = history.filter((h) => {
    const matchesSearch =
      h.checklist_templates?.title
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      h.submitted_by_profile?.full_name
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
    const matchesType =
      historyFilterType === "all" ||
      h.checklist_templates?.type === historyFilterType;
  ...
  return (
    ...
    >
      <div className="max-w-[1600px] mx-auto px-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-lg text-gray-500 font-medium">
                Compliance & Operational Verification
              </p>
            </div>
          </div>
        </motion.div>
        ...
        {/* Toolbar */}
        <div
          className={`${neumorphicCard} p-4 mb-8 flex flex-col xl:flex-row items-center justify-between gap-6 sticky top-24 z-30`}
        >
          <div className="flex items-center space-x-3 w-full xl:w-auto overflow-x-auto pb-2 xl:pb-0 scrollbar-hide">
            ...
            <button
              ...
              className={
                view === "history" ? neumorphicBtnActive : neumorphicBtn
              }
            >
              <History size={18} />
              <span>Logs</span>
            ...
            {canAccessAnalysis && (
              <button
                ...
                className={
                  view === "analysis" ? neumorphicBtnActive : neumorphicBtn
                }
              >
                <TrendingUp size={18} />
                <span>Analysis</span>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto">
            <div
              className={`flex-1 min-w-[280px] relative group ${neumorphicInset} px-4 py-2.5 flex items-center gap-3`}
            >
              <Search
                size={18}
          ...
          {view === "list" ? (
            <div className="space-y-16">
              {loading ? (
                ...
                <div
                  className={`${neumorphicCard} p-16 text-center shadow-inner`}
                >
                  ...
                  <p className="text-gray-500 mt-2">
                    Adjust search or initialize a new sequence.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  {["pre_exam", "post_exam", "custom"].map((type) => {
                    const typeTemplates = filteredTemplates.filter((t) =>
                      type === "custom"
                        ? t.type !== "pre_exam" && t.type !== "post_exam"
                        : t.type === type
                    );
                    ...
                    return (
                      <div key={type} className="space-y-8">
                        <div className="flex items-center gap-6">
                          <div
                            className={`flex items-center justify-center w-12 h-12 rounded-2xl shadow-sm border ${sectionMeta.borderColor}`}
                          >
                            <sectionMeta.icon
                              size={24}
                              className={
                                type === "pre_exam" ? "fill-current" : ""
                              }
                            />
                          </div>
                          <div className="flex-1">
                            <h2
                              className={`text-xl font-black uppercase tracking-[0.2em] ${sectionMeta.colorClass}`}
                            >
                              {sectionMeta.title}
                            </h2>
                            <p
                              className={`text-xs font-bold opacity-60 uppercase tracking-widest mt-0.5 ${sectionMeta.colorClass}`}
                            >
                              {sectionMeta.sub}
                            </p>
                          </div>
                          <div
                            className={`h-px flex-[2] bg-gradient-to-r ${sectionMeta.gradient} to-transparent`}
                          ></div>
                        </div>
            ...
            <div className="space-y-4 pb-20">
              {sortedDates.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-gray-500 mt-2">
                    No checklists have been submitted matching criteria.
                  </p>
                </div>
              ) : (
                      ...
                      {groupedHistory[dateKey].map((sub: any) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-5 w-full lg:w-auto min-w-[200px] pl-4">
                            <div
                              className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg transform group-hover:scale-110 transition-transform duration-300
                                                            ${
                                                              sub
                                                                .checklist_templates
                                                                ?.type ===
                          ...
                          <div className="flex-1 w-full text-center lg:text-left">
                            <div className="flex items-center justify-center lg:justify-start gap-3 mb-2">
                              ...
                              >
                                {sub.checklist_templates?.type === "pre_exam"
                                  ? "Start Shift"
                                  : sub.checklist_templates?.type ===
                                    "post_exam"
                                  ? "End Shift"
                                  : "Custom"}
                              </span>
                            </div>
                            <h4 className="text-xl font-bold text-gray-800 uppercase tracking-tight leading-tight">
                              {sub.checklist_templates?.title}
                            </h4>
                          </div>
...
const TemplateCard = ({
  ...
  const accentColor = isPre
    ? "text-blue-600"
    : isPost
    ? "text-purple-600"
    : "text-amber-600";
  const accentBg = isPre
    ? "bg-blue-500"
    : isPost
    ? "bg-purple-500"
    : "bg-amber-500";
  ...
  const neumorphicCard =
    "bg-[#e0e5ec] shadow-[9px_9px_16px_rgb(163,177,198,0.6),-9px_-9px_16px_rgba(255,255,255,0.5)] rounded-3xl border border-white/40";
  const innerShadow =
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-6">
        <div
          className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${innerShadow} ${accentColor} bg-[#e0e5ec]`}
        >
          <div
      ...
      <div className="flex-1 flex flex-col items-center text-center px-4 mb-8">
        <div
          className={`mb-4 w-12 h-1 bg-gradient-to-r from-transparent via-gray-300 to-transparent opacity-50`}
        ></div>
        ...
        <p className="text-gray-500 text-xs font-medium leading-relaxed line-clamp-2 max-w-[90%]">
          {template.description ||
            "Verified operational procedure ready for execution."}
        </p>
      </div>
        ...
        >
          <span
            className={`w-6 h-6 rounded-full flex items-center justify-center ${
              template.is_active
                ? "bg-gray-200 group-hover/btn:bg-blue-100 group-hover/btn:text-blue-600"
                : "bg-gray-100"
            }`}
          >
...
const HistoryItem = ({
  ...
  const typeLabel =
    submission.checklist_templates?.type === "pre_exam"
      ? "Pre-Exam"
      : submission.checklist_templates?.type === "post_exam"
      ? "Post-Exam"
      : "Custom";
  const typeColor =
    submission.checklist_templates?.type === "pre_exam"
    ...
    >
      <div className="flex-1">
        <div className="flex items-center gap-4 mb-3">
          <span
            className={`px-3 py-1 rounded-xl text-[8px] font-black uppercase tracking-[0.2em] border ${typeColor} shadow-sm`}
          >
            {typeLabel}
          </span>
        ...
        <div className="flex flex-col">
          <h4 className="text-lg font-black text-gray-700 uppercase group-hover:text-blue-700 transition-colors tracking-tight">
      ...
      <div className="flex items-center gap-5">
        <div
          className={`px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-[0.1em] flex items-center gap-2.5 shadow-sm border border-white/20 ${
        ...
        <div className="flex gap-3">
          <button
            ...
            className="p-3.5 rounded-2xl bg-[#e0e5ec] shadow-[4px_4px_8px_#bec3c9,-4px_-4px_8px_#ffffff] text-gray-500 hover:text-blue-600 transition-all hover:scale-105 active:scale-95"
          >
            <Eye size={20} />
          </button>
          <button
            ...
            className="p-3.5 rounded-2xl bg-[#e0e5ec] shadow-[4px_4px_8px_#bec3c9,-4px_-4px_8px_#ffffff] text-red-300 hover:text-red-500 transition-all hover:scale-105 active:scale-95"
          >
            <Trash2 size={20} />
          </button>
        </div>
...
const SubmissionDetailsModal = ({
  ...
  const themeGradient = isPre
    ? "from-blue-500 to-blue-600"
    : isPost
    ? "from-purple-500 to-purple-600"
    : "from-amber-500 to-amber-600";
  const themeLight = isPre
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
      ...
      >
        {/* Header Banner */}
        <div
          className={`relative px-8 py-8 bg-gradient-to-r ${themeGradient} text-white shrink-0`}
        >
          ...
          <div className="mt-8 flex items-center gap-8 text-xs font-bold bg-black/10 backdrop-blur-sm p-4 rounded-2xl border border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="opacity-50 text-[9px] uppercase tracking-widest mb-0.5">
                  Submitted By
                </p>
                <p className="text-lg leading-none">
                  {submission.submitted_by_profile?.full_name || "Unknown"}
                </p>
              </div>
            </div>
            ...
            {Object.entries(groupedQuestions).map(([section, questions]) => (
              <div key={section} className="flex flex-col gap-4">
                <div className="flex items-center gap-3 mb-2">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center shadow-inner ${themeLight}`}
                  >
                    <Box size={16} />
                  </div>
                    ...
                    if (q.type === "radio" || q.type === "dropdown") {
                      if (
                        ...
                        (answer.toLowerCase() === "no" ||
                          answer.toLowerCase() === "fail" ||
                          answer.toLowerCase() === "poor")
                      )
                        isNegative = true;
                    }
                      ...
                      >
                        <div className="flex justify-between items-start gap-4">
                          ...
                          {hasAnswer ? (
                            <div className={`shrink-0`}>
                              {q.type === "checkbox" ? (
                                ...
                                (answer.toLowerCase() === "yes" ||
                                  answer.toLowerCase() === "pass" ||
                                  answer.toLowerCase() === "good") ? (
                            ...
                            <span className="px-2 py-1 rounded bg-gray-100 text-gray-400 text-[9px] font-bold uppercase tracking-wider">
                              Skipped
                            </span>
                          )}
                        </div>
