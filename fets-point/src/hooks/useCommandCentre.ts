...
export const useDashboardStats = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const todaysRoster =
        todaysRosterData && todaysRosterData.length > 0
          ? {
              date: today,
              day: new Date().toLocaleDateString("en-US", { weekday: "short" }),
              staff: todaysRosterData.map(
                (r: any) => r.staff_profiles.full_name
              ),
            }
          : null;
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const trendPromises = Array.from({ length: 7 }).map((_, i) => {
        ...
        return applyBranchFilter(query, activeBranch).then(({ count }) => ({
          date: dateString,
          count: count ?? 0,
        }));
      });
...
export const useChecklistTemplates = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const preExam =
        templates.find((t) => t.type === "pre_exam") || DEFAULT_PRE_EXAM;
      const postExam =
        templates.find((t) => t.type === "post_exam") || DEFAULT_POST_EXAM;
