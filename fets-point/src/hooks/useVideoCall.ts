...
export function useVideoCall() {
  ...
  const createPC = useCallback(
    (targetUserId: string) => {
      ...
      pc.ontrack = (event) => {
        console.log(
          `[useVideoCall] Track received from ${targetUserId}:`,
          event.track.kind
        );
        ...
        setCallState((prev) => {
          ...
          const stream =
            event.streams[0] ||
            (existingStream ? existingStream : new MediaStream());
        ...
        switch (type) {
          case "offer":
            if (
              ...
              stateRef.current.isCalling ||
              stateRef.current.isReceivingCall
            ) {
            ...
            setCallState((prev) => ({
              ...
              callerId: from,
              callType: (payload as any).callType || "video",
            }));
            (window as any)._pendingOffer = offer;
            (window as any)._pendingCallType =
              (payload as any).callType || "video";
            break;

          case "answer":
            if (pcs.current[from]) {
              await pcs.current[from].setRemoteDescription(
          ...
          case "end-call":
            setCallState((prev) => {
              ...
              const stillInCall =
                Object.keys(newStreams).length > 0 || prev.isCalling;
              if (!stillInCall) {
