...
export const advancedConversationService = {
  ...
  async getConversationTimeline(
    ...
    try {
      const convWithMessages = await this.getConversationWithMessages(
        conversationId
      );
  ...
  async getSimilarConversations(
    ...
    try {
      ...
      const conversationIds = [
        ...new Set((data || []).map((d) => d.conversation_id)),
      ].slice(0, limit);
  ...
  async getEngagementMetrics(userId?: string): Promise<{
    ...
    try {
      ...
      let queryQuery = supabase
        .from("ai_query_log")
        .select("id, created_at, query_text");
...
export const insightsGeneratorService = {
  ...
  async generateInsights(userId?: string): Promise<{
    ...
    try {
      const metrics = await longitudinalAnalysisService.getEngagementMetrics(
        userId
      );
      const trends = await longitudinalAnalysisService.analyzeTrends(
        userId,
        "month"
      );
      ...
      if (metrics.totalQueries > 100) {
        insights.push(
          `You've made ${metrics.totalQueries} queries, showing active AI engagement`
        );
        if (metrics.avgSessionLength > 30) {
          insights.push(
            `Average session length of ${metrics.avgSessionLength} minutes indicates deep engagement`
          );
        }
      }
      ...
      if (trends.length >= 2) {
        ...
        if (recentTrend.trend_direction === "improving") {
          insights.push(
            "Your query patterns show improving trends with better response times"
          );
          recommendations.push(
            "Consider exploring more complex queries to leverage improved efficiency"
          );
        } else if (recentTrend.trend_direction === "declining") {
          insights.push("Query frequency has decreased recently");
          recommendations.push(
            "Try setting weekly goals for AI interactions to maintain engagement"
          );
        }
      }
      ...
      if (metrics.favoriteTopics.length > 0) {
        insights.push(
          `Top interests: ${metrics.favoriteTopics.slice(0, 3).join(", ")}`
        );
        recommendations.push(
          `Explore deeper topics in ${metrics.favoriteTopics[0]} for more insights`
        );
      }

      // Analyze activity patterns
      const peakHour = metrics.activityByHour.indexOf(
        Math.max(...metrics.activityByHour)
      );
      if (peakHour > 0) {
        insights.push(`Most active around ${peakHour}:00`);
        recommendations.push(
          `Schedule focused AI sessions during peak activity hours`
        );
      }
