...
async function fetchSummarizedData() {
  ...
  return {
    ...
    historical_summary: {
      ...
      open_incidents:
        recentIncidents?.filter((i: any) => i.status !== "closed").length || 0,
    ...
    exam_intelligence: {
      ...
      session_capacity:
        recentSessions?.reduce((acc: any, s: any) => {
    ...
    future_outlook: {
      ...
      upcoming_sessions:
        futureSessions?.slice(0, 10).map((s: any) => ({
          date: s.date,
...
export async function askClaude(userPrompt: string, userProfile?: any) {
  ...
  if (!apiKey || apiKey === "undefined" || apiKey.length < 10) {
    ...
    throw new Error(
      "System Alert: Claude Neural Key is missing. Please check your system configuration."
    );
  }
  ...
  try {
    ...
    if (error.message?.includes("rate limit")) {
      throw new Error(
        "Claude is at rate limit. Please wait a moment and try again."
      );
    }
