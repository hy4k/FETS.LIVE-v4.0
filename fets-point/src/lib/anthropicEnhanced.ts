...
async function safeFetch(promise: Promise<any>, tableName: string) {
  try {
    ...
    if (error) {
      console.warn(
        `[Context Warning] Failed to fetch ${tableName}:`,
        error.message
      );
      return [];
    }
...
export function parseAdvancedQuery(query: string) {
  ...
  for (const pattern of datePatterns) {
    ...
    if (match) {
      if (
        match[1]?.includes("last") ||
        match[1]?.includes("this") ||
  ...
  const queryIntents = [
    {
      patterns: ["how many", "count", "total", "number of", "statistics"],
      intent: "count",
    },
    {
      patterns: ["schedule", "when", "date", "timing", "upcoming", "future"],
      intent: "schedule",
    },
    {
      patterns: ["list", "show", "display", "give me", "what are"],
      intent: "list",
    },
    {
      patterns: ["compare", "difference", "vs", "versus", "comparison"],
      intent: "compare",
    },
    {
      patterns: ["trend", "growth", "increase", "decrease", "change over time"],
      intent: "trend",
    },
    {
      patterns: ["who", "which candidate", "candidate details"],
      intent: "candidates",
    },
    {
      patterns: ["pass rate", "pass percentage", "success rate", "results"],
      intent: "results",
    },
    {
      patterns: ["revenue", "income", "earnings", "payment", "fee"],
      intent: "revenue",
    },
  ];
...
async function fetchAllTimeHistoricalData(query: string) {
  ...
  if (branch && branch !== "all branches") {
    allSessionsQuery = allSessionsQuery.or(
      `branch_location.ilike.%${branch}%,branch.ilike.%${branch}%`
    );
  }
  ...
  const { data: allVault } = await supabase
    .from("fets_vault")
    .select("*")
    .eq("is_deleted", false);
  if (allVault) {
  ...
  const examKnowledgeBase = {
    ...
    CIA: {
      ...
      examFormat:
        "Part 1: Essentials of Internal Auditing, Part 2: Practice of Internal Auditing, Part 3: Business Knowledge",
      passingScore: "75%",
    ...
    CPA: {
      ...
      examFormat:
        "AUD (Auditing), BEC (Business Concepts), FAR (Regulation), REG (Taxation)",
      passingScore: "75%",
    ...
    EA: {
      ...
      examFormat:
        "Part 1: Individuals, Part 2: Business Entities, Part 3: Representation, Practice & Procedures",
      passingScore: "70%",
...
export function setupRealtimeSubscriptions(callback: (update: any) => void) {
  ...
  const sessionSub = supabase
    ...
    .on(
      "postgres_changes",
      { event: "INSERT", schema: "public", table: "calendar_sessions" },
      (payload) => {
...
export async function askClaude(userPrompt: string, userProfile?: any) {
  ...
  if (!apiKey)
    throw new Error("Neural Link Broken: No API key found in environment.");
  ...
  try {
    ...
    const { historicalData, parsedParams, metadata } =
      await fetchAllTimeHistoricalData(userPrompt);

    // Get current real-time data
    const [currentSessions, futureSessions, onlineStaff, branchStatus] =
      await Promise.all([
        safeFetch(
        ...
        safeFetch(
          supabase.from("branch_locations").select("*").eq("is_active", true),
          "branch_status"
        ),
      ]);

    const operator =
      userProfile?.full_name || userProfile?.name || "Unidentified Operator";

    // Calculate statistics
    const totalRevenue =
      historicalData.allSessions?.reduce(
        (acc: number, s: any) => acc + (s.fees_collected || s.revenue || 0),
        0
      ) || 0;

    const sessionCapacity =
      historicalData.allSessions?.reduce((acc: any, s: any) => {
    ...
    const contextData = {
      ...
      recent_activity: {
        ...
        last_24h_candidates:
          historicalData.allCandidates?.filter((c: any) => {
    ...
    const systemInstruction = `
            ...
            ALL-TIME STATISTICS:
            ${
              contextData.historical.sessions.all_time_total
                ? `- Total Sessions Ever: ${contextData.historical.sessions.all_time_total}`
                : ""
            }
            ${
              contextData.historical.candidates.all_time_total
                ? `- Total Candidates Ever: ${contextData.historical.candidates.all_time_total}`
                : ""
            }
            ${
              contextData.historical.incidents.all_time_total
                ? `- Total Incidents Ever: ${contextData.historical.incidents.all_time_total}`
                : ""
            }
            ${
              contextData.historical.financials.total_revenue_estimate
                ? `- Estimated Total Revenue: ₹${contextData.historical.financials.total_revenue_estimate.toLocaleString()}`
                : ""
            }
            ...
            BY BRANCH:
            ${JSON.stringify(
              contextData.historical.sessions.by_branch,
              null,
              2
            )}
            ...
            - Online Staff: ${
              contextData.realtime.online_staff.names.join(", ") || "None"
            }
            ...
            - Date Range: ${
              parsedParams.dateRange
                ? `${parsedParams.dateRange.start} to ${parsedParams.dateRange.end}`
                : "All Time"
            }
            ...
            ${Object.keys(contextData.exam_knowledge)
              .map((exam) => `- ${exam}`)
              .join("\n")}
    ...
    console.log(
      "🧠 FETS SUPREME OMNI: Processing with ALL-TIME data + real-time..."
    );
    ...
    if (error.message?.includes("rate limit")) {
      throw new Error(
        "Claude is at maximum capacity. Please wait and try again."
      );
    }
    throw new Error(
      `Supreme Neural Engine Error: ${error.message || "Unknown"}`
    );
  }
}
