...
async function fetchAllTemporalData() {
  ...
  const [
    ...
    // ONLINE STAFF
    safeFetch(
      supabase.from("staff_profiles").select("*").eq("is_online", true),
      "online_staff"
    ),

    // ACTIVE CANDIDATES (Today)
    safeFetch(
      supabase.from("candidates").select("*").eq("exam_date", today),
      "active_candidates"
    ),
  ...
  const examStats = processExamStatistics(
    allExamsConducted,
    allCandidates,
    allSessions
  );
 ...
 */
function processExamStatistics(
  examsConducted: any[],
  candidates: any[],
  sessions: any[]
) {
  const examTypeMap: Record<
    string,
    { conducted: number; registered: number; scheduled: number }
  > = {};
...
export async function askGemini(userPrompt: string, userProfile?: any) {
  ...
  if (!apiKey || apiKey === "undefined" || apiKey.length < 10) {
    ...
    throw new Error(
      "System Alert: AI Neural Key is missing. Please check your system configuration."
    );
  }
  ...
  try {
    ...
    const contextData = {
      ...
      exam_intelligence: {
        ...
        conducted_summary:
          temporalData.historical.examsConducted?.slice(0, 50) || [],
        upcoming_exams: temporalData.future.exams || [],
        candidate_registrations: temporalData.historical.candidates || [],
        session_capacity:
          temporalData.historical.sessions?.reduce((acc: any, s: any) => {
      ...
      aggregates: {
        ...
        open_incidents:
          temporalData.historical.incidents?.filter(
            (i: any) => i.status !== "closed"
          ).length || 0,
        active_vault_docs: temporalData.historical.vault?.length || 0,
        pending_leaves:
          temporalData.historical.leaves?.filter(
    ...
    const systemInstruction = `
            ...
            Operator: ${contextData.metadata.operator.name} (${
      contextData.metadata.operator.role
    })
    ...
    throw new Error(
      `Neural Engine Malfunction: ${error.message || "Unknown Core Error"}`
    );
  }
}
